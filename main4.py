...
class BaseAIModel:
    ...
    def extract_features(self, prediction: Dict[str, Any]) -> np.ndarray:
...
class MobileNetModel(BaseAIModel):
    ...
    def extract_features(self, prediction: Dict[str, Any]) -> np.ndarray:
        ...
        return prediction.get('features', self.extract_features_from_prediction(
            prediction.get('class', 'unknown'), 
            prediction.get('confidence', 0.0)
        ))
    
    def extract_features_from_prediction(self, class_name: str, confidence: float) -> np.ndarray:
...
class GeminiModel(BaseAIModel):
    ...
    # In both MobileNetModel and GeminiModel, change:
    def extract_features_from_prediction(self, class_name: str, confidence: float) -> np.ndarray:
        features = np.zeros(Config.FEATURE_DIMENSIONS)
        features[0] = confidence
        ...
        return features  # Convert to list here
        ...
        if self.main_model.available:
            try:
                ...
                if main_prediction.get('confidence', 0) >= self.confidence_threshold:
                    ...
                    print(f"⚠️ Main model low confidence: {main_prediction['confidence']:.2f}, trying fallback...")
            except Exception as e:
...
async def startup_event():
    ...
    if success:
        ...
        print("⚠️ Some models failed to load, but application will continue")
...
async def predict_label_api(image: UploadFile = File(...), label: str = Form("")):
    ...
    try:
        ...
        return JSONResponse({
            "predicted_label": prediction['class'],
            "confidence": float(prediction['confidence']),  # Ensure float, not numpy float
